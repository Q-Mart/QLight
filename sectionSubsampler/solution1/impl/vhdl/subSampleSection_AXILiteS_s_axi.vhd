...
entity subSampleSection_AXILiteS_s_axi is
generic (
...
end entity subSampleSection_AXILiteS_s_axi;
...
architecture behave of subSampleSection_AXILiteS_s_axi is
    ...
    constant ADDR_FACTOR_DATA_0   : INTEGER := 16#18#;
    constant ADDR_FACTOR_CTRL     : INTEGER := 16#1c#;
    ...
    signal int_factor          : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_FACTOR_DATA_0 =>
                        rdata_data <= RESIZE(int_factor(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    factor               <= STD_LOGIC_VECTOR(int_factor);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FACTOR_DATA_0) then
                    int_factor(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_factor(31 downto 0));
                end if;
            end if;
        end if;
    end process;
