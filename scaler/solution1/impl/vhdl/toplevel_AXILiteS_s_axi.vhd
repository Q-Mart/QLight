...
architecture behave of toplevel_AXILiteS_s_axi is
    ...
    constant ADDR_SCALEDLENGTH_DATA_0 : INTEGER := 16#30#;
    constant ADDR_SCALEDLENGTH_CTRL   : INTEGER := 16#34#;
    ...
    signal int_scaledLength    : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SCALEDLENGTH_DATA_0 =>
                        rdata_data <= RESIZE(int_scaledLength(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    scaledLength         <= STD_LOGIC_VECTOR(int_scaledLength);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SCALEDLENGTH_DATA_0) then
                    int_scaledLength(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_scaledLength(31 downto 0));
                end if;
            end if;
        end if;
    end process;
