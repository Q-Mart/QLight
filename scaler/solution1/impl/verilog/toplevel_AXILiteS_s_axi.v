...
// 0x2c : reserved
// 0x30 : Data signal of scaledLength
//        bit 31~0 - scaledLength[31:0] (Read/Write)
// 0x34 : reserved
...
localparam
    ...
    ADDR_SCALEDLENGTH_DATA_0 = 6'h30,
    ADDR_SCALEDLENGTH_CTRL   = 6'h34,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_SCALEDLENGTH_DATA_0: begin
                    rdata <= int_scaledLength[31:0];
                end
            endcase
        end
    end
...
assign scaledLength = int_scaledLength;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RAM_DATA_0)
            int_ram[31:0] <= (WDATA[31:0] & wmask) | (int_ram[31:0] & ~wmask);
    end
end
...
// int_scaledLength[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_scaledLength[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_SCALEDLENGTH_DATA_0)
            int_scaledLength[31:0] <= (WDATA[31:0] & wmask) | (int_scaledLength[31:0] & ~wmask);
    end
end
